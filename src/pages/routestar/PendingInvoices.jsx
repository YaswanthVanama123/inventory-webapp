...
const PendingInvoices = () => {
  ...
  const handleClearInvoices = async () => {
    ...
    try {
      ...
      console.error('Error deleting pending invoices:', err);
      showError(err.message || 'Failed to delete pending invoices');
    } finally {
      setDeleting(false);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-gray-400 mt-1">
              Sales invoices from RouteStar (Pending)
            </p>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-600 dark:text-gray-400">
                    Clear all pending invoices from the database ({totalItems} invoices)
                  </p>
                </div>
                <Button
                  onClick={() => setShowDeleteConfirm(true)}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden">
        {invoices.length === 0 ? (
          <EmptyState
            ...
            description="No pending invoices match your current filters"
            action={
              isAdmin ? (
